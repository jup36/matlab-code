function saveEvent(binFile)
%SVAEVENT Reads IMEC binary data and makes event file
%   SAVEVENT(BF) reads BF file and makes event file using bin file. It also
...
% data location
IMEC_DATA_PATH = 'E:\';
% IMEC_DATA_PATH = 'F:\zigzag\data_ephys\';
BCS_DATA_PATH = ['C:\Users\', getenv('USERNAME'), '\OneDrive - Howard Hughes Medical Institute\project\neozig\data_behavior\'];

if nargin < 1 || exist(binFile, 'file')==0
    [binName, binPath] = uigetfile([IMEC_DATA_PATH, '\*.imec.ap.bin'], 'Choose binary data file');
    if binName==0; return; end
    binFile = fullfile(binPath, binName);
end

% load event file
meta = readOption(binFile);
dataFile = replace(binFile, '.bin', ['_imec3_opt', num2str(meta.imProbeOpt), '_data.mat']);
...
if ~(exist(dataFile, 'file')==2)
    ...
    if ~all(ismember(checkVariable1, eventFileInfo))
        doCalcImec = 'Yes';
    else
    ...
    checkVariable2 = {'Vr'};
    if ~all(ismember(checkVariable2, eventFileInfo))
        doCalcBcs = 'Yes';
    else
